...
void JRoute::handle_events(SDL_Event* event, Camera* cam) {
    ...
    if (event->type == SDL_MOUSEBUTTONDOWN) {
        if (event->button.button == SDL_BUTTON_LEFT)
            if (!loadingRoute)
                start_route_loading((cam->getView().x + event->button.x), (cam->getView().y + event->button.y));
    ...
    if (event->type == SDL_MOUSEMOTION) {
        if ((event->motion.state == SDL_PRESSED)) {
            if (loadingRoute)
                make_waypoint((cam->getView().x + event->button.x), (cam->getView().y + event->button.y));
            handledMovement = true;
        }
        lastXpos = cam->getView().x + event->button.x;
        lastYpos = cam->getView().y + event->button.y;
    }
}
